...
LONG DetectNCR(volatile struct ncr710 *ncr)
{
	...
	*gary = 0x00;
	...
	dbgprintf("  Setting DCNTL.EA bit...\n");
	...
	*gary = 0x80;
	...
	if (gary_timeout & 0x80) {
		dbgprintf("ERROR: NCR chip not detected (GARY timeout)\n");
		dbgprintf("  The hardware may not be present\n");
		return -1;
	}

	dbgprintf("  GARY timeout check passed\n");

	// Now we can safely read ISTAT register
	dbgprintf("  Reading ISTAT register...\n");
	...
	if (istat_before == 0xFF && istat_after == 0xFF) {
		dbgprintf("WARNING: NCR chip not detected (bus reads 0xFF)\n");
		dbgprintf("  This may be an emulator without NCR hardware emulation\n");
		return -1;
	}

	dbgprintf("NCR chip detected (ISTAT=0x%02lx)\n", (ULONG)istat_before);
	return 0;
}
...
LONG ResetNCR(volatile struct ncr710 *ncr)
{
	...
	dbgprintf("  Disabling burst bus mode...\n");
	...
	dbgprintf("  Configuring chip test register 0...\n");
	ncr->ctest0 = CTEST0F_BTD | CTEST0F_EAN | CTEST0F_ERF;

	dbgprintf("  NCR chip reset complete\n");

	// Clear any pending interrupts from reset/abort
	dbgprintf("  Clearing pending interrupts...\n");
	...
	dbgprintf("NCR reset complete (ISTAT=0x%02lx)\n", (ULONG)ncr->istat);
	return 0;
}
...
{
	dbgprintf("Initializing NCR 53C710 for DMA testing...\n");
	...
	dbgprintf("  Disabling SCSI interrupts...\n");
	ncr->sien = 0;
	...
	// FC2 = function code
	dbgprintf("  Configuring DMA mode...\n");
	...
	dbgprintf("  Configuring DMA control...\n");
	ncr->dcntl = DCNTLF_EA | DCNTLF_COM;
	...
	dbgprintf("  Disabling DMA interrupts (using polling mode)...\n");
	ncr->dien = 0;
	...
	dbgprintf("  Disabling SCSI bus operations...\n");
	ncr->scid = 0;
	ncr->scntl0 = 0;
	ncr->scntl1 = 0;
	...
	dbgprintf("NCR initialization complete\n");
	dbgprintf("  DMODE:  0x%02lx\n", (ULONG)ncr->dmode);
	dbgprintf("  DCNTL:  0x%02lx\n", (ULONG)ncr->dcntl);
	dbgprintf("  DIEN:   0x%02lx (interrupts disabled)\n", (ULONG)ncr->dien);
...
LONG CheckNCRStatus(volatile struct ncr710 *ncr, const char *context)
{
	...
	if (istat & ISTATF_DIP) {
		...
		if (dstat & DSTATF_IID) {
			dbgprintf("ERROR [%s]: Illegal Instruction Detected (DSTAT=0x%02lx)\n",
			        context, (ULONG)dstat);
		...
		if (dstat & DSTATF_ABRT) {
			dbgprintf("ERROR [%s]: Script Aborted (DSTAT=0x%02lx)\n",
			        context, (ULONG)dstat);
			return -1;
		}

		if (dstat & DSTATF_WTD) {
			dbgprintf("ERROR [%s]: Watchdog Timer Expired (DSTAT=0x%02lx)\n",
			        context, (ULONG)dstat);
			return -1;
		}
	...
	if (istat & ISTATF_SIP) {
		dbgprintf("WARNING [%s]: Unexpected SCSI interrupt (ISTAT=0x%02lx)\n",
		        context, (ULONG)istat);
		// Clear SCSI status
		(void)ncr->sstat0;
		(void)ncr->sstat1;
